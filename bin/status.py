...
def printStatus():
    ...
    if all(len(l) == 0 for l in [added,missing,untracked,modified,conflict,binary,large]):
        ...
        for c,f in sorted(added+missing+untracked+modified+conflict+binary+large, key=lambda i: i[1]):
            print c, f
        print
